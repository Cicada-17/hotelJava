...
<body>
	...
	<div class="container-fluid content">
		...
		<div class="main">
			...
			<div class="row">
				...
				</div><h3 class="page-header"><i class="fa fa-star"></i> 固定工资管理</h3>
					<ol class="breadcrumb">
						<li><i class="fa fa-home"></i><a href="<%=basePath%>index.html">首页</a></li>
						<li><i class="fa fa-star"></i>固定工资管理</li>						  	
					</ol>
			</div>
    ...
    <script type="text/javascript">
    	...
    	{
    			var temp=/^\d+(\.\d+)?$/;
 				var z = $("input[name='jiben']").val();
 				var d = $("input[name='cainuan']").val();
    		if( $("input[name='jiben']").val()==""||$("input[name='cainuan']").val()=="")
    		{
    			alert("不能为空");
    		}
    		else if(temp.test(z)==false||temp.test(d)==false){
    		...
    		$.ajax({
                    type:"post",
                    url:"<%=basePath%>updgwage",  //需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
                    data:{//设置数据源
                    	staffid:   $("p[id='staffid']").text(),
                    	jiben: $("input[name='jiben']").val(),
                    	cainuan: $("input[name='cainuan']").val()
    	...
    	function xiugai(id)
    	{
    		...
    		$.ajax({
                    type:"post",
                    url:"<%=basePath%>findgwagebyid",  //需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
                    data:{//设置数据源
                    	staffid:id,
        ...
        function s(){
                ...
                $.ajax({
                    type:"post",
                    url:"<%=basePath%>money2_liu",  //需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
                    data:{//设置数据源
                    ...
                    success:function(data){
                        ...
                        //得到的d是一个形如{"key":"value","key1":"value1"}的数据类型，然后取值出来
                      	...
                      	var listgwage = msg["listgwage"];
                      	var liststaff = msg["liststaff"];
                      
                      	for( var gwage in listgwage)
                      	{	
                      		
                      		asd+="<tr ><td>";
                      		asd+=listgwage[gwage].staffid;
                      		asd+="</td><td>";
                      		...
                      		asd+="</td><td>";
                      		asd+=listgwage[gwage].jiben;
                      		asd+="</td><td>";
                      		asd+=listgwage[gwage].cainuan;
		...
		<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							<h2><i class="fa fa-star red"></i><span class="break"></span><strong>固定工资<form name="reg_testdate" action="" method="post"> 
		...
		<div class="modal-dialog" >
			<div class="modal-content" >
				<div class="modal-header">
					...
					<h4 class="modal-title">修改固定工资</h4>
				</div>
							...
							<form action="" method="post" enctype="multipart/form-data" class="form-horizontal ">
				                ...
				                <div class="form-group">
				                   <div class="form-group">
				                    ...
				                    <div class="col-md-9">
				                        <input type="text" name="jiben"/>
				                    </div>
				                    </div>
				                </div>
				                <div class="form-group">
				                 <div class="form-group">
				                    <label class="col-md-3 control-label">采暖补贴</label>
				                    <div class="col-md-9">
				                        <input type="text" name="cainuan"/>
				                    </div>
				                    </div>
